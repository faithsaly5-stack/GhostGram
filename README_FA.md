...
می‌توانید روح‌گرام را بدون هزینه و بدون داشتن سرور اختصاصی، به صورت ۲۴ ساعته روی پلتفرم‌های ابری اجرا کنید:

#### الف) استقرار روی [Railway.app](https://railway.app):
1. **احراز هویت اولیه**: یک‌بار دستور `python login.py` را روی سیستم شخصی اجرا کنید تا فایل سشن تلگرام (`teleagent_session.session`) ساخته شود.
2. مخزن را روی گیت‌هاب خود Fork کرده و در سایت Railway یک **New Service > GitHub Repo** بسازید.
3. در تب **Variables** مقادیر فایل `.env` خود (`API_ID`, `API_HASH`, `PHONE_NUMBER`, `OWNER_ID`, `OWNER_NAME`, `GEMINI_API_KEYS` و...) را اضافه کنید.
4. دستور اجرا (**Start Command**) را روی `python main.py` قرار دهید (یا اجازه دهید با `Dockerfile` موجود بیلد شود).
5. ایجنت شما به صورت ۲۴ ساعته در کلود فعال است!

#### ب) استقرار روی [Render.com](https://render.com):
1. در پنل رندر یک سرویس از نوع **Background Worker** بسازید و مخزن را متصل کنید.
2. محیط را روی **Python 3** یا **Docker** بگذارید.
3. متغیرهای محیطی خود را در تب **Environment** وارد کنید.
4. دستور **Start Command** را `python main.py` بگذارید و دکمه **Deploy** را بزنید!
