...
- **چرخش خودکار کلیدها (Round-Robin)**: پشتیبانی از تعداد نامحدود کلید جمینای در `.env` (با جداکننده کاما).
...
### 🌟 روش اول: اجرای تک‌کلیک روی ویندوز (بدون نیاز به سرور یا کدنویسی)
اگر قصد دارید بات را خیلی ساده روی سیستم شخصی خود اجرا کنید و سرور یا VPS ندارید:

1. سورس پروژه را دانلود یا کلون کنید.
2. روی فایل **`run.bat`** (یا `setup.bat`) دوبار کلیک کنید.
3. یک ویزارد تعاملی باز شده و کلیدهای تلگرام، جمینای و مشخصات نام/بیوگرافی شما را می‌پرسد و به تلگرام متصل می‌شود.
4. **تمام!** از این پس هر زمان که خواستید بات روی سیستمتان فعال باشد، کافیست روی **`run.bat`** دوبار کلیک کنید.

---

### 💻 روش دوم: نصب دستی و محلی (Windows / macOS / Linux / Termux)
...
# ۴. اجرای ویزارد تنظیمات (یا ویرایش دستی فایل .env)
python setup.py

# ۵. ورود و احراز هویت در تلگرام
python login.py
...
### ☁️ روش سوم: استقرار رایگان و ۲۴ ساعته روی کلود (Railway / Render / Koyeb - بدون نیاز به خرید سرور)
می‌توانید روح‌گرام را بدون نیاز به روشن ماندن کامپیوتر یا خرید سرور VPS، به صورت ۱۰۰٪ رایگان و ۲۴ ساعته روی پلتفرم‌های ابری اجرا کنید.

#### 🔑 مرحله ۱: دریافت کد سشن (Session String) - یک‌بار انجام می‌شود
از آنجایی که سرورهای ابری ترمینال تعاملی ندارند تا در زمان اجرا کد پیامکی تلگرام را از شما بپرسند، یک‌بار کد سشن تلگرام خود را روی کامپیوتر شخصی تولید می‌کنیم:
1. دستور `python login.py` را روی کامپیوتر خود اجرا کنید (یا روی `run.bat` دوبار کلیک کنید).
2. شماره تلفن و کد تایید تلگرام را وارد کنید.
3. پس از ورود موفق، برنامه یک کد متنی طولانی به نام **`SESSION_STRING`** (شروع با `1Ap...`) چاپ می‌کند. آن را کپی کنید!

---

#### 🚀 مرحله ۲: استقرار روی [Railway.app](https://railway.app) (پیشنهاد شده)
1. این مخزن را در اکانت گیت‌هاب خود Fork کرده یا کلون و پوش کنید.
2. وارد سایت [Railway.app](https://railway.app) شوید $\leftarrow$ روی **New Project** بزنید $\leftarrow$ گزینه **Deploy from GitHub repo** را انتخاب و مخزن `GhostGram` را انتخاب کنید.
3. در داشبورد سرویس، به تب **Variables** بروید و متغیرهای زیر را وارد کنید:
   - `API_ID`: کد عددی API تلگرام
   - `API_HASH`: هش API تلگرام
   - `PHONE_NUMBER`: شماره تلفن تلگرام
   - `SESSION_STRING`: کد سشن کپی‌شده از مرحله ۱
   - `GEMINI_API_KEYS`: کلید(های) هوش مصنوعی جمینای
   - `OWNER_ID`: آیدی عددی تلگرام
   - `OWNER_NAME`: نام نمایشی
   - `OWNER_BIO`: بیوگرافی یا شغل
4. پلتفرم Railway به صورت خودکار با `Dockerfile` پروژه را بیلد کرده و روح‌گرام را ۲۴ ساعته فعال می‌سازد!

---

#### 🚀 مرحله ۳: یا استقرار روی [Render.com](https://render.com)
1. در سایت [Render.com](https://render.com) روی **New +** بزنید $\leftarrow$ گزینه **Background Worker** را انتخاب کنید.
2. مخزن گیت‌هاب خود را متصل کنید.
3. نوع محیط را **Docker** بگذارید (یا **Python 3** با دستور بیلد `pip install -r requirements.txt` و دستور استارت `python main.py`).
4. در تب **Environment Variables** متغیرهای خود (به خصوص `SESSION_STRING`) را وارد نمایید.
5. دکمه **Deploy** را بزنید تا روح‌گرام ۲۴ ساعته در کلود فعال گردد!

---

### 🖥️ روش چهارم: استقرار ۲۴ ساعته روی سرور لینوکس شخصی (VPS Deploy)
اگر سرور لینوکس (VPS) دارید و می‌خواهید بات به عنوان یک سرویس پس‌زمینه پایدار `systemd` همیشه فعال باشد:

1. در فایل `.env` مشخصات اتصال SSH به سرور را وارد کنید:
   ```env
   VPS_IP=your.vps.ip.here
   SSH_USER=root
   SSH_PORT=22
   ```
2. روی فایل **`deploy.bat`** دوبار کلیک کنید (یا در لینوکس `./deploy.sh` را اجرا کنید).
3. اسکریپت به صورت خودکار پروژه را بسته‌بندی کرده، آپلود می‌کند، پایتون و نیازمندی‌ها را نصب کرده و سرویس `ghostgram.service` را با قابلیت اجرای مجدد خودکار پس از ریبوت فعال می‌سازد.

---

### 🐳 روش پنجم: اجرا با داکر (Docker & Docker Compose)
...
# =================================================================
# 👤 مشخصات هویتی و پرسونای شما
# =================================================================
OWNER_ID=123456789
OWNER_NAME=نام_شما
OWNER_BIO=دانشجو و برنامه‌نویس
OWNER_WEBSITE=yourwebsite.com
OWNER_SERVICES=مشاوره، برنامه‌نویسی و طراحی وب
OWNER_INTERESTS=موسیقی، تکنولوژی، کتاب و گفتگو
...
## 🔒 امنیت
 ...
 > [!CAUTION]
 > **هرگز فایل‌های `.env` یا `*.session` را در گیت‌هاب عمومی آپلود نکنید!**
